...
public class ProfileService : IProfileService
{
    ...
    public async Task<ProfileWithAdsDto> GetProfileAsync(User user)
    {
        ...
        {
            dto.Favorites = await _context.Favorites
                .Where(f => f.UserId == user.Id && !f.IsDeleted)
                .Include(f => f.Advertisement).ThenInclude(a => a.Photoes)
                .Select(f => new AdvertisementDto
                {
                    Id = f.Advertisement.Id,
                    Title = f.Advertisement.Title,
                    Body = f.Advertisement.Body,
                    Price = f.Advertisement.Price,
                    CreateDate = f.Advertisement.CreateDate,
                    Photoes = f.Advertisement.Photoes
                        .Where(p => !p.IsDeleted)
                        .Select(p => new AdvertisementPhotoDto
                        {
                            Id = p.Id,
